    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'LynXzy Lagi Cape :/' : banned ? 'kamu dibanned' : 'ya?',
                '',
...
Note: Syarat dan Ketentuan mungkin berlaku`.trim(), wm, 'Payment', '#payment', 'Owner', '#owner', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`Im LynXzy 🤖 || ⏲️ Aktif selama ${uptime} || 👥 User : ${Object.keys(global.db.data.users).length} User || 👾 Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'}`).catch(_ => _)
